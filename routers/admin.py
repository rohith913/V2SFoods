...
def admin_register_page(request: Request):
    return templates.TemplateResponse(request=request, name="admin_register.html", context={})


@router.post("/register")
def admin_register(
    request: Request,
    admin_name: str = Form(...), mobile: str = Form(...),
    email: str = Form(...), password: str = Form(...),
    db: Session = Depends(get_db)
):
    existing = db.query(AdminMaster).filter(AdminMaster.email == email).first()
    if existing:
        return templates.TemplateResponse(request=request, name="admin_register.html",
                                          context={"error": "Admin email already registered"})
    admin = AdminMaster(admin_name=admin_name, mobile=mobile, email=email,
...
def admin_login(
    ...
    admin = db.query(AdminMaster).filter(
        AdminMaster.email == email, AdminMaster.status == "ACTIVE"
    ).first()
...
def all_orders(request: Request, db: Session = Depends(get_db)):
    if not request.session.get("admin_id"):
        return RedirectResponse("/admin/login")
    orders = db.query(OrderMaster).order_by(OrderMaster.id.desc()).all()
    return templates.TemplateResponse(request=request, name="all_orders.html",
